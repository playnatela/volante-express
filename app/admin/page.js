...
import { 
  ...
  ArrowRightLeft, Pencil, CheckCircle2, AlertTriangle
} from 'lucide-react';
import { 
...
export default function AdminPage() {
  ...
  const [selectedRegion, setSelectedRegion] = useState('all'); // Padrão: Visão Global
  ...
  const [editingItem, setEditingItem] = useState(null); // Para editar Serviços/Despesas/Usuários
  const [showEditModal, setShowEditModal] = useState(false);
  const [modalType, setModalType] = useState(''); // 'appointment', 'expense', 'installer'
  ...
  const [showAddForm, setShowAddForm] = useState(false); // Estoque
  ...
  async function fetchData() {
    ...
    // Filtros Dinâmicos (Se for 'all', não filtra região)
    const regionFilter = selectedRegion === 'all' ? {} : { region_id: selectedRegion };
    
    // 1. Instalações (Serviços)
    let appQuery = supabase.from('appointments').select('*').eq('status', 'concluido').like('completed_at', `${selectedMonth}%`);
    if (selectedRegion !== 'all') appQuery = appQuery.eq('region_id', selectedRegion);
    const { data: apps } = await appQuery;
    setAppointments(apps || []);

    // 2. Despesas
    let expQuery = supabase.from('expenses').select('*, expense_categories(name)').like('date', `${selectedMonth}%`);
    if (selectedRegion !== 'all') expQuery = expQuery.eq('region_id', selectedRegion);
    const { data: exps } = await expQuery;
    setExpenses(exps || []);

    // 3. Contas (Se Global, mostra todas. Se Regional, mostra globais + da região)
    let accQuery = supabase.from('accounts').select('*').order('name');
    if (selectedRegion !== 'all') accQuery = accQuery.or(`region_id.is.null,region_id.eq.${selectedRegion}`);
  ...
  function openEditModal(type, item) {
    setModalType(type);
    setEditingItem({ ...item }); // Copia para não alterar direto na lista
  ...
  async function handleSaveEdit() {
    ...
    // Remove campos que não devem ser enviados no update (joins, etc)
    const payload = { ...editingItem };
    delete payload.expense_categories; 
  ...
  // --- FINANCEIRO: Transferência e Contas ---
  async function handleAddAccount(e) {
    ...
    const { error } = await supabase.from('accounts').insert([{ 
        ...
        balance: newAccount.initial_balance // Saldo Inicial
    }]);
  ...
  async function handleTransfer(e) {
    ...
    await supabase.from('accounts').update({ balance: fromAcc.balance - amount }).eq('id', transferData.from);

    // 2. Põe no Destino
    const { data: toAcc } = await supabase.from('accounts').select('balance').eq('id', transferData.to).single();
    await supabase.from('accounts').update({ balance: toAcc.balance + amount }).eq('id', transferData.to);
  ...
  async function handleAddExpense(e) {
    e.preventDefault();
    const reg = selectedRegion === 'all' ? 'divinopolis' : selectedRegion; // Fallback se estiver no global
    ...
    if (!error) {
        ...
        setNewExpense({ description: '', amount: '', category_id: '', account_id: '' });
        fetchData();
    }
  }
  ...
  const dashboardStats = useMemo(() => {
    ...
    // Gráfico Barra: Clientes por Dia
    const clientsByDay = {};
    ...
    return { 
        totalIncome, totalExpense, profit: totalNetIncome - totalExpense, 
        pieData, barData 
    };
  }, [appointments, expenses]);

  // Cálculo Comissões (Global ou Regional)
                ...
                <div className="space-y-4">
                    ...
                    {modalType === 'installer' && (
                        <>
                            ...
                            <div><label className="text-xs font-bold text-slate-500">Comissão Fixa (R$)</label><input type="number" className="w-full p-3 border rounded-xl" value={editingItem.commission_rate} onChange={e => setEditingItem({...editingItem, commission_rate: e.target.value})} /></div>
                            <p className="text-xs text-red-500 mt-2">* Para alterar email/senha, use o painel do Supabase.</p>
                        </>
                    )}
                </div>
      ...
      {showTransferModal && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
            <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl animate-in zoom-in-95">
                <h3 className="text-lg font-bold mb-4 text-slate-900 flex items-center gap-2"><ArrowRightLeft size={20}/> Nova Transferência</h3>
                <form onSubmit={handleTransfer} className="space-y-4">
                    <div><label className="text-xs font-bold text-slate-500">De (Origem)</label><select className="w-full p-3 border rounded-xl" required onChange={e => setTransferData({...transferData, from: e.target.value})}><option value="">Selecione...</option>{accounts.map(a => <option key={a.id} value={a.id}>{a.name} (R$ {a.balance})</option>)}</select></div>
            ...
            <div className="flex w-full md:w-auto gap-3">
                ...
                <div className="flex-1 md:flex-none flex items-center gap-2 bg-slate-800 px-3 py-2 rounded-xl border border-slate-700">
                    ...
                    <select value={selectedRegion} onChange={e => setSelectedRegion(e.target.value)} className="bg-transparent font-bold text-slate-200 outline-none cursor-pointer w-full uppercase">
                        <option value="all" className="text-black font-bold">🌐 Visão Global (Todas)</option>
                ...
                <div className="max-w-6xl mx-auto animate-in fade-in space-y-6">
                        ...
                        <>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 h-80">
                                    <h4 className="font-bold text-slate-700 mb-6 flex items-center gap-2 text-sm uppercase tracking-wide"><PieIcon size={18}/> Despesas por Categoria</h4>
                                ...
                                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 h-80">
                                    <h4 className="font-bold text-slate-700 mb-6 flex items-center gap-2 text-sm uppercase tracking-wide"><BarChart3 size={18}/> Clientes por Dia</h4>
                                    <ResponsiveContainer width="100%" height="85%">
                    ...
                    {activeTab === 'financeiro' && (
                        <div className="space-y-6">
                            {/* SALDOS E TRANSFERÊNCIA */}
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                                <div className="flex justify-between items-center mb-4">
                                    <h3 className="font-bold text-slate-800 flex items-center gap-2"><Banknote size={20} className="text-blue-600"/> Saldos</h3>
                                    <button onClick={() => setShowTransferModal(true)} className="text-xs bg-slate-100 hover:bg-slate-200 text-slate-700 px-3 py-2 rounded-lg font-bold flex items-center gap-2 transition-colors"><ArrowRightLeft size={14}/> Transferir</button>
                                </div>
                            ...
                            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                                <table className="w-full text-sm text-left">
                                    <thead className="bg-slate-50 text-slate-500 font-medium border-b border-gray-100"><tr><th className="p-4">Data</th><th className="p-4">Descrição</th><th className="p-4 text-right">Valor</th><th className="p-4 text-center">Ações</th></tr></thead>
                                    <tbody className="divide-y divide-gray-100">
                                        {[...appointments.map(a => ({...a, type: 'in', date: a.completed_at, val: a.gross_amount, label: `${a.vehicle_model} - ${a.customer_name}`})), 
                                          ...expenses.map(e => ({...e, type: 'out', date: e.date, val: e.amount, label: e.description}))]
                                ...
                                {commissionReport.map((rep, i) => (
                                    <div key={i} className="bg-slate-50 p-5 rounded-2xl border border-slate-200 group relative">
                                        ...
                                        <div className="border-t border-slate-200 pt-4 flex justify-between items-center">
                                            <span className="text-slate-400 text-xs font-medium">A Pagar</span>
                                ...
                                <div className="space-y-6">
                                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                                        ...
                                        <form onSubmit={handleAddAccount} className="space-y-4">
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                <div><label className="text-xs font-bold text-slate-500 mb-1 block">Tipo</label><select className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl" value={newAccount.type} onChange={e => setNewAccount({...newAccount, type: e.target.value})}><option value="banco">Banco Global</option><option value="carteira">Caixa Regional</option></select></div>
                                                <div><label className="text-xs font-bold text-slate-500 mb-1 block">Saldo Inicial</label><input type="number" className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl" value={newAccount.initial_balance} onChange={e => setNewAccount({...newAccount, initial_balance: e.target.value})} /></div>
                                            </div>
                                    ...
                                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                                        ...
                                        <div className="flex gap-2 mb-4">
                                            <input className="flex-1 p-2 border rounded-lg text-sm" placeholder="Nova Categoria..." value={newCategory} onChange={e => setNewCategory(e.target.value)} />
                                            <button onClick={handleAddCategory} className="bg-blue-600 text-white px-4 rounded-lg font-bold text-sm">Add</button>
                                        </div>
                                        <div className="flex flex-wrap gap-2">
                                            {categories.map(c => <span key={c.id} className="bg-slate-100 text-slate-600 px-3 py-1 rounded-full text-xs font-bold flex items-center gap-2">{c.name} <button onClick={() => handleDelete('expense_categories', c.id)} className="text-slate-400 hover:text-red-500"><X size={12}/></button></span>)}
                                        </div>
                                    </div>
                                </div>
