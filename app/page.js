...
export default function HomePage() {
  ...
  const [activeTab, setActiveTab] = useState('agenda'); // agenda | extrato | perfil
  ...
  async function checkUser() {
    ...
    // 1. PORTEIRO INTELIGENTE (Com exceção para o modo Preview)
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 pb-32 font-sans">
      ...
      {/* HEADER FIXO (Só muda o título dependendo da aba) */}
      <div className="bg-slate-900 pt-10 pb-8 rounded-b-[40px] shadow-2xl border-b border-slate-800 flex flex-col items-center justify-center relative overflow-hidden">
        ...
        {activeTab === 'agenda' && (
            <>
                ...
                {isPreviewMode && <span className="bg-amber-500 text-black text-[10px] font-bold px-2 py-0.5 rounded mt-2">MODO ESPIÃO (ADMIN)</span>}
            </>
        )}

        {activeTab === 'perfil' && (
            <div className="text-center z-10">
                <div className="w-24 h-24 bg-slate-800 rounded-full mx-auto mb-4 border-4 border-slate-700 flex items-center justify-center relative">
                    <User size={40} className="text-slate-500"/>
                    <button className="absolute bottom-0 right-0 bg-blue-600 p-2 rounded-full text-white hover:bg-blue-500"><Camera size={14}/></button>
                </div>
      ...
      {/* --- CONTEÚDO PRINCIPAL (Muda com a aba) --- */}
        ...
        {activeTab === 'agenda' && (
            <>
                <div className="flex items-center justify-between">
                    <h3 className="text-slate-400 text-xs font-bold uppercase tracking-wider flex items-center gap-2"><Calendar size={14}/> Serviços Pendentes</h3>
                    ...
                    {appointments.map(app => (
                        <div key={app.id} onClick={() => router.push(`/atendimento/${app.id}`)} className="bg-slate-900 p-4 rounded-2xl border border-slate-800 relative overflow-hidden active:scale-95 transition-transform cursor-pointer hover:border-blue-500/30 shadow-md">
                            <div className="absolute top-0 right-0 bg-slate-800 pl-3 pr-2 py-1 rounded-bl-xl border-b border-l border-slate-700 flex items-center gap-1">
                                ...
                                <span className="text-[10px] font-bold text-slate-300 uppercase tracking-wide truncate max-w-[100px]">{app.calendar_name || app.region_id || 'Regional'}</span>
                            </div>
                            <div className="flex items-start gap-4 mt-2">
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="inline-flex items-center gap-1.5 bg-slate-950/50 px-2 py-1 rounded-lg border border-slate-800/50">
                                        ...
                                        <span className="text-xs font-medium text-slate-300">{formatDate(app.appointment_at)}</span>
                                    </div>
                                </div>
            ...
            <div className="space-y-4">
                <button onClick={handleUpdatePassword} className="w-full bg-slate-900 p-4 rounded-2xl border border-slate-800 flex items-center justify-between group hover:border-blue-500/50 transition-colors">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="text-left"><h4 className="text-slate-200 font-bold">Alterar Senha</h4><p className="text-xs text-slate-500">Atualize sua segurança</p></div>
                    </div>
                ...
                <button onClick={handleLogout} className="w-full bg-red-950/20 p-4 rounded-2xl border border-red-900/30 flex items-center justify-between group hover:bg-red-950/30 transition-colors">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="text-left"><h4 className="text-red-400 font-bold">Sair da Conta</h4><p className="text-xs text-red-500/60">Encerrar sessão no aparelho</p></div>
                    </div>
                </button>
                
                <p className="text-center text-xs text-slate-600 pt-8">Versão 2.1.0 • Volante Pro</p>
            </div>
        )}
      ...
      {/* FAB (Só na Agenda) */}
      {activeTab === 'agenda' && (
          <button onClick={handleNewService} disabled={creating} className="fixed bottom-24 right-6 w-14 h-14 bg-blue-600 hover:bg-blue-500 rounded-2xl shadow-lg shadow-blue-600/30 flex items-center justify-center text-white transition-transform active:scale-90 z-30">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-slate-900/95 backdrop-blur-md border-t border-slate-800 pb-6 pt-2 px-6 z-40">
        <div className="flex justify-around items-center">
            <button onClick={() => setActiveTab('agenda')} className={`flex flex-col items-center gap-1 p-2 transition-colors ${activeTab === 'agenda' ? 'text-blue-500' : 'text-slate-500'}`}>
                <ListTodo size={24} strokeWidth={activeTab === 'agenda' ? 2.5 : 2} />
            ...
            <button onClick={() => setActiveTab('perfil')} className={`flex flex-col items-center gap-1 p-2 transition-colors ${activeTab === 'perfil' ? 'text-blue-500' : 'text-slate-500'}`}>
